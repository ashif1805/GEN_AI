...
import com.metlife.ace.automation.config.webelement.*;
import com.metlife.ace.automation.config.webelement.BasePageElement;
import com.metlife.ace.automation.config.webelement.BenefitsPageElement;
import com.metlife.ace.automation.config.webelement.StatusCodesPageElements;
import com.metlife.ace.automation.model.*;
import com.metlife.ace.automation.repository.*;
...
import com.metlife.ace.automation.model.Deferred;
import com.metlife.ace.automation.model.LifeAnnuity;
import com.metlife.ace.automation.model.PaymentPreviewReportLT;
import com.metlife.ace.automation.repository.AnnuitantInPayBeneRepository;
import com.metlife.ace.automation.repository.AnnuitantInPayJointAnnuitantRepository;
import com.metlife.ace.automation.repository.DeferredRepository;
import com.metlife.ace.automation.repository.PaymentPreviewReportLTRepository;
import com.metlife.ace.automation.repository.LifeAnnuityRepository;
...
@Slf4j
public abstract class ACEBasePage extends GenreicPage {
	@Autowired
	...
	@LazyAutowired
	private BulkCheckInPayLifeRepository  BulkCheckInPayLife;
	@LazyAutowired
	...
	@LazyAutowired
	private DeceasedAnnuitantWInitialJARepository DeceasedAnnuitantWInitialJARepository;
	@LazyAutowired
	...
	@LazyAutowired
	private InPayAnnuitantTwoBeneRepository InPayAnnuitantTwoBeneRepository;
	@LazyAutowired
	private InPayAnnuitantTwoBeneBulkRepository InPayAnnuitantTwoBeneBulkRepository;
...
//Added to select correct entity (Contract ID) among multiple options
	...
	public String searchesforQuery(String testCaseNo, String query) {
		...
		try {
			...
			}else if (query.equalsIgnoreCase("DeceasedAnnuitantWInitialJA")) {
					List<DeceasedAnnuitantWInitialJA> DeceasedAnnuitantWInitialJAList = new ArrayList<>();

					// List parametersList = new ArrayList();
					// parametersList.add(Altid);
					DeceasedAnnuitantWInitialJAList = DeceasedAnnuitantWInitialJARepository.getDeceasedAnnuitantWInitialJA(queryDetails);// this

					log.info("ACE Details : " + DeceasedAnnuitantWInitialJAList);

					for (DeceasedAnnuitantWInitialJA DeceasedAnnuitantWInitialJAiterator : DeceasedAnnuitantWInitialJAList) {
						AltID = DeceasedAnnuitantWInitialJAiterator.getPersonAltId();
						log.info("Annuitant's Alt Id : " + AltID);
			...
			else if (query.equalsIgnoreCase("BulkCheckInPayLife")) {
				List<BulkCheckInPayLife> BulkCheckInPayLifeList = new ArrayList<>();

				// List parametersList = new ArrayList();
				// parametersList.add(Altid);
				BulkCheckInPayLifeList = BulkCheckInPayLife.getBulkCheckInPayLife(queryDetails);// this

				log.info("ACE Details : " + BulkCheckInPayLifeList);

				for (BulkCheckInPayLife BulkCheckInPayLifeiterator : BulkCheckInPayLifeList) {
					AltID = BulkCheckInPayLifeiterator.getPersonAltId();
					log.info("Annuitant's Alt Id : " + AltID);
		...
		}else if (query.equalsIgnoreCase("InPayAnnuitantTwoBene")) {
			List<InPayAnnuitantTwoBene> InPayAnnuitantTwoBeneList = new ArrayList<>();

			// List parametersList = new ArrayList();
			// parametersList.add(Altid);
			InPayAnnuitantTwoBeneList = InPayAnnuitantTwoBeneRepository.getInPayAnnuitantTwoBene(queryDetails);// this

			log.info("ACE Details : " + InPayAnnuitantTwoBeneList);

			for (InPayAnnuitantTwoBene InPayAnnuitantTwoBene : InPayAnnuitantTwoBeneList) {
				AltID = InPayAnnuitantTwoBene.getPersonAltId();
				log.info("Annuitant's Alt Id : " + AltID);

			}
		}else if (query.equalsIgnoreCase("InPayAnnuitantTwoBeneBulk")) {
			List<InPayAnnuitantTwoBeneBulk> InPayAnnuitantTwoBeneBulkList = new ArrayList<>();

			// List parametersList = new ArrayList();
			// parametersList.add(Altid);
			InPayAnnuitantTwoBeneBulkList = InPayAnnuitantTwoBeneBulkRepository.getInPayAnnuitantTwoBeneBulk(queryDetails);// this

			log.info("ACE Details : " + InPayAnnuitantTwoBeneBulkList);

			for (InPayAnnuitantTwoBeneBulk InPayAnnuitantTwoBeneBulk : InPayAnnuitantTwoBeneBulkList) {
				AltID = InPayAnnuitantTwoBeneBulk.getPersonAltId();
				log.info("Annuitant's Alt Id : " + AltID);
...
public void resetBatchFilter() throws Exception {
	...
	public void nonUSMailingAddressmodification(String testCaseNo) throws Exception {
		waitForPageLoad();
		moveToDefault();
		Thread.sleep(4000);
		this.clickEdit();
		waitForPageLoad();
		String[] mockNonUSAddress = this.getMockNonUSAddress();
		Boolean addressAddUpdateFlag= false;

		switchToFrame("tapestry");

		conditionalWait(this.batchpage.getAddressGearButton(), "elementToBeVisible");
		conditionalWait(this.batchpage.getAddressGearButton(), "presenceOfElementLocated");
		conditionalWait(this.batchpage.getAddressGearButton(), "elementToBeClickable");
		this.scrollWindow(this.batchpage.getAddressGearButton());
		this.getElementByXpath(this.batchpage.getAddressGearButton()).click();
		this.waitForPageLoad();

		if(this.findElements(this.batchpage.getAddMailingButton()).size()>0){
			log.info("Foreign Mailing Address needs to be added");
			addressAddUpdateFlag=true;

			conditionalWait(this.batchpage.getAddMailingButton(), "elementToBeVisible");
			conditionalWait(this.batchpage.getAddMailingButton(), "presenceOfElementLocated");
			conditionalWait(this.batchpage.getAddMailingButton(), "elementToBeClickable");
			this.getElementByXpath(this.batchpage.getAddMailingButton()).click();
			this.waitForPageLoad();
			Thread.sleep(4000);
		} if(this.findElements(this.batchpage.getMailingAddressTable()).size()>0) {
			log.info("Foreign Mailing Address needs to be updated");
			addressAddUpdateFlag=true;

			conditionalWait(this.batchpage.getMailingThreeDotButton(), "elementToBeVisible");
			conditionalWait(this.batchpage.getMailingThreeDotButton(), "presenceOfElementLocated");
			conditionalWait(this.batchpage.getMailingThreeDotButton(), "elementToBeClickable");
			this.getElementByXpath(this.batchpage.getMailingThreeDotButton()).click();
			this.waitForPageLoad();
			Thread.sleep(2000);

			conditionalWait(this.batchpage.getMailingEditButton(), "elementToBeVisible");
			conditionalWait(this.batchpage.getMailingEditButton(), "presenceOfElementLocated");
			conditionalWait(this.batchpage.getMailingEditButton(), "elementToBeClickable");
			this.getElementByXpath(this.batchpage.getMailingEditButton()).click();
			this.waitForPageLoad();
			Thread.sleep(4000);
